...
- **Knowledge Distillation**: A smaller YOLO model is trained using the pre-labeled data, which allows for more efficient operation during the actual classification stage.
...
- **Classification**: After extraction, the cloth item is passed into the classification instance. Here, a decision tree model is used to determine the specific category of the item based on features like texture, color, and material.
...
- **Classification**: For both pipelines, extracted cloth items are classified using either a decision tree (production) or a smaller YOLO model (pre-annotation).
...
3. Detected cloth items are extracted and passed into the classification models (YOLOs for pre-annotation, decision tree for production).
...
Its important to higlight here the **playground** scripts as they are useful to investigate and debug certain instances from the actual pipeline.
...
- **helper/**: Includes helper functions or scripts that are reused across different modules.
- **clip.py**: Implements CLIP (Contrastive Language-Image Pretraining) functionality for visual tasks.
- **cloth_categories.py**: Defines and handles the classification of various cloth categories.
- **cloth_matrix.py**: Manages the matrix structure for cloth data, to.
- **mongo.py**: Handles MongoDB-related operations such as data storage and retrieval.
- **sam.py**: Implements a Segment Anything Model (SAM) or and functionality for image segmentation.
- **yolo.py**: Implements the YOLO (You Only Look Once) object detection algorithm.
- **yolos.py**: Implementation of the YOLOS model
- 
Each script contributes to the overall pipeline by enabling preprocessing, detection, or classification tasks, interacting with the main classification models.
